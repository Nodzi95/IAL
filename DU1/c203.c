...
*/
    if(q != NULL){
        int i;
        for(i = 0; i < MAX_QUEUE; i++){     //naplnìní prvkù pole znaky '*'
            q->arr[i] = '*';
        }
        q->f_index = 0;                     //nastavení ukazatele zaèátku na 0. index
        q->b_index = 0;                     //nastavení ukazatele na první volný prvek na 0. index
    }
...
*/
    return (index + 1) % QUEUE_SIZE;        //výpoèet pro získání dal¹ího indexu
}
...
*/
    if(q->b_index == q->f_index) return 1;  //pokud se index prvního prvku rovná indexu prvnímu volnému, tak je fronta prázdná, pak vrací 1
    else return 0;
}
...
*/
    if(q->f_index == nextIndex(q->b_index)) return 1;   //pokud se index prvního prvku rovná dal¹ímu indexu za prvním volným, tak je fronta plná, pak vrací 1
    else return 0;
}
...
*/
    if(!queueEmpty(q)){                 //pokud je fronta prázdná volá error funkci
        *c = q->arr[q->f_index];        //do c ulo¾í prvek ze zaèátku fronty
    }
...
*/
    if(!queueEmpty(q)){                                 //pokud je fronta prázdná volá error funkci
        q->f_index = nextIndex(q->f_index);             //posune zaèátek fronty
        //if(q->f_index > q->b_index) q->f_index = 0;
    }
...
*/
    if(!queueEmpty(q)){             //pokud je fronta prázdná volá error funkci
        queueFront(q, c);           //do c ulo¾í prvek ze zaèátku fronty
        queueRemove(q);             //posune zaèátek fronty
    }
...
*/
    if(!queueFull(q)){                          //pokud je fronta plná volá error funkci
        q->arr[q->b_index] = c;                 //na první volnou polo¾ku vlo¾í c
        q->b_index = nextIndex(q->b_index);     //posune index první volné polo¾ky
    }
