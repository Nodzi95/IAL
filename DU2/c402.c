...
**/
    ...
    else
    {
        ...
        {
            newPtr3 = newPtr;
            if(newPtr->Cont > Content)
            {
                newPtr = newPtr->LPtr;
            }
            else if(newPtr->Cont < Content)
            {
                newPtr = newPtr->RPtr;
            }
            else return;
        }
        newPtr2 = malloc(sizeof(struct tBTNode));
        if(newPtr2 == NULL)
        {
            printf("Chyba alokace pameti\n");
            return;
        }
        newPtr2->Cont = Content;
        newPtr2->LPtr = NULL;
        newPtr2->RPtr = NULL;
        if(newPtr3->Cont > Content) newPtr3->LPtr = newPtr2;
        else if(newPtr3->Cont < Content) newPtr3->RPtr = newPtr2;
    }
}
...
**/
    ...
    while(!SEmptyP(&Stack))
    {
        newPtr = STopP(&Stack)->RPtr;
        if(newPtr != NULL) Leftmost_Preorder(newPtr, &Stack);
        else
        {
            newPtr = STopPopP(&Stack);
            if(!SEmptyP(&Stack))
            {
                while((newPtr->Cont) > (STopP(&Stack)->Cont))
                {
                    SPopP(&Stack);
                    if(SEmptyP(&Stack)) break;
                }
            }
        }
    }
...
**/
    ...
    while(!SEmptyP(&Stack))
    {
        ...
        newPtr = STopP(&Stack)->RPtr;
        if(newPtr != NULL) Leftmost_Inorder(newPtr, &Stack);
        else
        {
            newPtr = STopPopP(&Stack);
            if(!SEmptyP(&Stack))
...
**/
    ...
    while(!SEmptyP(&Stack))
    {
        ...
        {
            if(max < SSizeP(&Stack)) max = SSizeP(&Stack);
            newPtr = STopPopP(&Stack);
            if(!SEmptyP(&Stack))
            {
                while((newPtr->Cont) > (STopP(&Stack)->Cont))
...
**/
    ...
    do
    {
        if(*RootPtr == NULL)
        {
            if(!SEmptyP(&Stack)) *RootPtr = STopPopP(&Stack);
        }
        else
        {
            if((*RootPtr)->RPtr != NULL) SPushP(&Stack, (*RootPtr)->RPtr);
            newPtr = *RootPtr;
