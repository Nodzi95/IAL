...
**/
	L->Act = NULL;          //inicializace ukazatele na aktuální a první buòku
    L->First = NULL;
}
...
***/
    tElemPtr ptr1;          //vytvoøení pomocného ukazatele
    L->Act = NULL;          //nastavení ukazatele na aktuální buòku do stavu po inicializaci
    ptr1 = L->First;        //
    while(ptr1){            //dokud ukazatel na první buòku nebude NULL, tak bude uvolòovat buòky z pamìti
        L->First = ptr1->ptr;   //ulo¾ení ukazatele na následující buòku
        free(ptr1);             //uvolnìní
        ptr1 = L->First;
    }
}
...
** volá funkci Error().
**/
    tElemPtr ptr1;  //vytvoøení nového ukazatele
    if((ptr1 =  malloc(sizeof(*L))) == NULL){    //naalokování pamìti o velikosti struktury buòky (tElemPtr)
        Error();
        return;
    }
    else{
        ptr1->data = val;       //ulo¾í hodnotu val do nové buòky
        ptr1->ptr = L->First;   //novì vytvorená buòka bude ukazovat tam, kam ukazoval ukazatel na zaèátek
        L->First = ptr1;        //novì vytvoøená buòka bude první buòka v seznamu
    }
}
...
**/
    L->Act = L->First;          //nastavení aktivity na první prvek
}
...
**/
    if(L->First != NULL) *val = L->First->data;     //ulo¾í data z první buòky do val
    else Error();
}
...
**/
    if(L->First != NULL){               //o¹etøení neprázdnosti seznamu
        tElemPtr ptr1;
        if(L->Act == L->First){         //pokud byl první mazaný prvek aktivní, tak se aktivita ztrácí
            L->Act = NULL;
        }
        ptr1 = L->First;                //nastavení ukazatelù
        L->First = L->First->ptr;
        free(ptr1);                     //uvolnìní buòky z pamìti
    }
}	
...
** nic se nedìje.
**/
    if((L->Act != NULL) && (L->Act->ptr != NULL)){  //o¹etøení neaktivního seznamu a toho, ¾e je buòka za aktivní buòkou (je co mazat)
        tElemPtr ptr1;
        ptr1 = L->Act->ptr;                         //nastavení ukazatelù
        L->Act->ptr = ptr1->ptr;
        free(ptr1);                                 //uvolnìní buòky z pamìti
    }
}
...
**/
    if(L->Act != NULL){
        tElemPtr ptr1;
        ptr1 =  malloc(sizeof(*L));         //alokace pamìti pro novou buòku
        if(ptr1 == NULL){
            Error();
            return;
        }
        else{
            ptr1->data = val;               //nastavení ukazatelù
            ptr1->ptr = L->Act->ptr;
            L->Act->ptr = ptr1;
        }
    }
...
**/
    if(L->Act != NULL) *val = L->Act->data; //do val ulo¾í data z aktivní buòky
    else Error();
}
...
**/
    if(L->Act != NULL){
        L->Act->data = val;                 //pøepí¹e data v aktivní buòky
    }
}
...
**/
    if(L->Act != NULL){
        if(L->Act->ptr == NULL) L->Act = NULL;  //pokud byla aktivní buòka zároveò poslední, tak seznam ztrácí aktivitu
        else L->Act = L->Act->ptr;              //nastavení aktivity na dal¹í buòku
    }
}
...
**/
    if(L->Act != NULL) return 1;                //pokud je seznam aktivní vrací 1
