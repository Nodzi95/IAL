...
**/
    if(RootPtr != NULL)
    {
        ...
        {
            if(RootPtr->Key > K) BSTSearch(RootPtr->LPtr, K, Content);
            else BSTSearch(RootPtr->RPtr, K, Content);
        }
    }
    else return false;
}
...
**/
    ...
    {
        if((*RootPtr)->Key > K) BSTInsert(&((*RootPtr)->LPtr), K, Content);
        else if((*RootPtr)->Key < K) BSTInsert(&((*RootPtr)->RPtr), K, Content);
...
**/
    ...
    {
        pom1 = BSTHeight(NodePtr->LPtr, true);
        pom2 = BSTHeight(NodePtr->RPtr, true);
        if(pom1 > pom2) max = pom1 + 1;
        else max = pom2 + 1;
    }
    return max;
}
